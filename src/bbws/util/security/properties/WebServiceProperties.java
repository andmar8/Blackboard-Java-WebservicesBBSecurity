...
public class WebServiceProperties
{
	...
	try
	{
	    ...
	    sre = ((SystemRegistryEntryDbLoader)PersistenceServiceFactory.getInstance().getDbPersistenceManager().getLoader(SystemRegistryEntryDbLoader.TYPE)).loadByKey(vendorId+"-"+applicationHandle+"-"+param);
	}
    ...
    //MAG = Method Access Group

    /**
     * This is used to get the MAG key for the given MAG name;
     * if the user is using oauth authentication, the MAG key acts
     * as the oauth consumer_secret, the MAG name acts as the oauth
     * consumer_key
     * @param mag
     * @return
     * @throws Exception
     */
    public String getMAGKeyForMAG(String mag) throws Exception
    {
        PropertiesBean pb = new PropertiesBean();
        Enumeration e = pb.getProperties(this.applicationHandle).propertyNames();
        String prop;
        while(e.hasMoreElements())
        {
            prop = (String)e.nextElement();
            if(prop.matches(mag+".magkey"))
            {
                return pb.getProperty(this.applicationHandle,prop);
            }
        }
        return "";
    }

    public String whichMAGMatchesMAGKey(String magKey) throws Exception
    {
        PropertiesBean pb = new PropertiesBean();
        Enumeration e = pb.getProperties(this.applicationHandle).propertyNames();
        String prop;
        while(e.hasMoreElements())
        {
            ...
            if(prop.contains(".magkey"))
            {
                ...
                if(pb.getProperty(this.applicationHandle,prop).equals(magKey))
                {
