...
public class PropertiesBean implements java.io.Serializable
{
	...
	//private static String applicationHandle = ""; //Name of bb application
	...
	public void setProperties(String applicationHandle,HttpServletRequest request) throws Exception
	{
		...
		try
		{
		    //sre = ((SystemRegistryEntryDbLoader)BbServiceManager.getPersistenceService().getDbPersistenceManager().getLoader(SystemRegistryEntryDbLoader.TYPE)).loadByKey(vendorId+"-"+applicationHandle+"-"+key);
                    sre = ((SystemRegistryEntryDbLoader)PersistenceServiceFactory.getInstance().getDbPersistenceManager().getLoader(SystemRegistryEntryDbLoader.TYPE)).loadByKey(vendorId+"-"+applicationHandle+"-"+key);
		}
		...
		if(knf || !sre.getValue().equals(value))
		{
			...
			sre.setDescription(vendorId+applicationHandle);
                        ...
                        ((SystemRegistryEntryDbPersister)PersistenceServiceFactory.getInstance().getDbPersistenceManager().getPersister(SystemRegistryEntryDbPersister.TYPE)).persist(sre);
	...
	public Properties getProperties(String applicationHandle) throws Exception
	{
		...
		Registry r = ((SystemRegistryEntryDbLoader)PersistenceServiceFactory.getInstance().getDbPersistenceManager().getLoader(SystemRegistryEntryDbLoader.TYPE)).loadRegistry();
	...
	public String getProperty(String applicationHandle,String propertyName)
	{
            String _prop = "";
            //System.err.println("Attempting to get property "+propertyName);
            try
            {
                    Properties prop = this.getProperties(applicationHandle);
                    if(prop!=null)
                    {
